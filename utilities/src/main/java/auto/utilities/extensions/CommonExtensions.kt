...
/**
 * Checks if given [receiver] is not null and if so, calls the specified function [block]
 * with the given [receiver] as its receiver and returns its result.
 */
fun <T, R> safeWith(receiver: T?, block: T.() -> R): R? {
    return if (receiver != null) block(receiver) else null
}
